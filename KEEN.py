...
class Error:
    ...
    def as_string(self):
        result = f'In file {self.pos_start.fn}, and on line {self.pos_start.ln + 1}'
...
class Parser:
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'return', 'continue', 'break' or an expression"))
        return res.success(expr)

    def expr(self):
        res = ParseResult()

        if self.current_tok.matches(TT_KEYWORD, 'stor'):
            res.register_advancement()
            self.advance()
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'stor', 'if', 'for', 'while', 'fun', int, float, identifier, '+', '-', '(', '[' or 'not'"
            ))
    ...
    def subbed_by(self, other):
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, RTError(
                    other.pos_start, other.pos_end,
                    'Element could not be removed from list, index is out of bounds',
                    self.context
                )
        else:
            return None, Value.illegal_operation(self, other)
    ...
    def dived_by(self, other):
        if isinstance(other, Number):
            try:
                ...
                return None, RTError(
                    other.pos_start, other.pos_end,
                    'Element could not be retrieved from list, index is out of bounds',
                    self.context
                )
        else:
            return None, Value.illegal_operation(self, other)
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_append(self, exec_ctx):
        ...
        if isinstance(value, List):
            if value.elements == list_.elements:
                return RTResult().failure(RTError(
                    self.pos_start, self.pos_end,
                    "Lists cannot be appended to themselves, try using '+' instead",
                    exec_ctx
                ))
    ...
    def execute_ule(self, exec_ctx):
        list_ = exec_ctx.symbol_table.get("list")
        index = exec_ctx.symbol_table.get("index")
        value = exec_ctx.symbol_table.get("value")
        print('list=', list_, 'index=', index, 'value=',
              value, 'element=', list_.elements[index.value])
        ...
        if not isinstance(index, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "Second argument must be index value",
                exec_ctx
            ))
        ...
        try:
            ...
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Element at this index could not be updated because index is out of bounds',
                exec_ctx
            ))
        return RTResult().success(element)
    execute_ule.arg_names = ["list", "index", "value"]
...
BuiltInFunction.ule = BuiltInFunction("ule")
...
global_symbol_table.set("Nice", Number(42069))
...
global_symbol_table.set("ULE", BuiltInFunction.ule)
